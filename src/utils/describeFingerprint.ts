...
export interface DescribeFingerprintOptions<Value> {
  ...
  validate: (value: Value, originalValue: Value, scope: Scope) => void | Promise<void>
}

export function describeFingerprint<Value> (title: string, opts: DescribeFingerprintOptions<Value>) {
  describe(title, () => {
    let scope: Scope
