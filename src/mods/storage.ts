...
export function modifyStorage (scope: Scope) {
  modifyFunctionReturnValue(scope.StorageManager.prototype, 'estimate', async ({ originalReturnValue, random }) => {
    // Await the original value to prevent a timing attack
    const estimate = await originalReturnValue;
