...
declare global {
  interface DeprecatedStorageQuota {
    queryUsageAndQuota(
      onResult: (usage: number, quota: number) => void,
      onError: () => void,
    ): void
  }
...
export function modifyStorage({
  ...
  modifyReturned(
    ...
    async ({ originalReturned, random }) => {
      ...
      estimate.quota =
        2 * 1024 * 1024 * 1024 +
        Math.floor(random.nextFloat() * 30 * 1024 * 1024 * 1024)
  ...
  if ('webkitTemporaryStorage' in scope.navigator) {
    const webkitTemporaryStorageProto = Object.getPrototypeOf(
      scope.navigator.webkitTemporaryStorage,
    )
    modifyValue(webkitTemporaryStorageProto, 'queryUsageAndQuota', () => {
      return function (
        this: DeprecatedStorageQuota,
        onResult: (usage: number, quota: number) => void,
        onError?: () => void,
      ) {
        void scope.StorageManager.prototype.estimate
          .call(scope.navigator.storage)
          .then((estimate) => {
