...
export function modifyWebGL({ scope, modifyReturned, forEachType }: Modifier) {
  forEachType(
    [scope.WebGLRenderingContext, scope.WebGL2RenderingContext],
    (type) => {
      modifyReturned(
        type.prototype,
        'getParameter',
        ({ originalReturned, originalArgs, random }) => {
          if (originalReturned) {
            ...
            switch (paramId) {
              ...
              case 36347: // MAX_VERTEX_UNIFORM_VECTORS
                return Math.max(
                  0,
                  originalReturned -
                    random.mutateByUInt32(paramId).nextIntBetween(0, 4),
                )
